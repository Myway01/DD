...
void SignupWidget::on_pushButton_signup_clicked()
{
    ...
    QString card = this->ui->lineEdit_card->text();
    ...
    if (card.length() == 0){
        this->ui->label_error->setText("请输入您的车牌号码！");
        this->ui->lineEdit_card->setFocus();
        return;
    }
    if (psw.length() < 6){
        if(psw.length() == 0){
    ...
    connect(this, SIGNAL(signup_sig(QString,QString,QString,QString,QString)), worker, SLOT(signupDrv(QString,QString,QString,QString,QString)));
    connect(worker, SIGNAL(numret(int)), this, SLOT(signup_slot(int)));
    emit signup_sig(username, nickname, card, psw, paypsw);
